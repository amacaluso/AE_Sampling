...
for key in sorted(counts):
    print("%s: %s" % (key, counts[key]))
    quantum1.append(counts[key])

# Computation of dataset considering it after normalisation
# and measuring the qubits
quantum1 = [v/n_shots for v in quantum1]
classical = [v.real**2 for v in desired_vector]
# Compute the difference between the two dataset
# 'classical vs quantum'
R = [np.round(x1 - x2, 4) for (x1, x2) in zip(quantum1, classical)]
print(R)
average_error = np.mean([abs(el) for el in R])
## Plot the comparison between the two datasets
...
for key in sorted(counts):
    print("%s: %s" % (key, counts[key]))
    quantum2.append(counts[key])

# Computation of dataset considering it after normalisation
# and measuring the qubits
quantum2 = [v/n_shots for v in quantum2]
classical = [v.real**2 for v in desired_vector]
# Compute the difference between the two dataset
# 'classical vs quantum'
R = [np.round(x1 - x2, 4) for (x1, x2) in zip(quantum2, classical)]
print(R)
average_error = np.mean([abs(el) for el in R])
## Plot the comparison between the two datasets
...
for key in sorted(counts):
    print("%s: %s" % (key, counts[key]))
    quantum3.append(counts[key])

# Computation of dataset considering it after normalisation
# and measuring the qubits
quantum3 = [v/n_shots for v in quantum1]
classical = [v.real**2 for v in desired_vector]
# Compute the difference between the two dataset
# 'classical vs quantum'
R = [np.round(x1 - x2, 4) for (x1, x2) in zip(quantum3, classical)]
print(R)
average_error = np.mean([abs(el) for el in R])
## Plot the comparison between the two datasets
for i in range(0,N,2):
...
for i in range(0,4,2):
    plt.scatter(quantum1[i], quantum1[i+1], color = 'green', alpha=0.5)
    plt.scatter(quantum2[i], quantum2[i+1], color = 'blue', alpha=0.5)
    plt.scatter(quantum3[i], quantum3[i+1], color = 'black', alpha=0.5)
plt.title('Scatter plot')
plt.xlabel('x_1')
plt.ylabel('x_2')
plt.show()
